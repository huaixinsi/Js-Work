...
一个基于 **React + TypeScript + Express + MySQL + Docker** 的电影推荐与票务系统课程项目。系统区分普通用户和管理员，普通用户负责浏览电影、选座购票和本地评论，管理员负责票房统计、用户管理、订单管理、数据报表和同步真实热映电影。
...
- `POST /api/movies/sync-tmdb`：管理员同步 TMDB 热映电影
